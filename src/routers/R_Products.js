...
router.post('/add', function (req, res) {
    var name, origin, price, cateName, imageName, info, imageNameTrue, dateOfM, flag = 1;
    name = req.body.name;
    ...
    if (flag == 1) {
        const obj = {
            name, origin, price, imageName, cateName, info, dateOfM
        }
...
router.post('/upLoadFile', function (req, res) {
    ...
    upload(req, res, function (err) {
        if (err instanceof multer.MulterError) {
            ...
            res.send({ kq: 1, msg: 'updload sucessfully', imageNameSend, imageNameReal });
        }
    })
});
...
router.post('/update', function (req, res) {
    var id, name, origin, price, cateName, imageName, info, dateOfM, flag = 1;
    ...
    if (flag == 1) {
        ...
        if (imageName == '') {
            obj = {
                name, origin, price, cateName, info, dateOfM
            }
        } else {
            obj = {
                name, origin, price, imageName, cateName, info, dateOfM
            ...
            .exec((err, data) => {
                if (err) {
                    ...
                    if (imageName == '') {
                        ...
                        try {
                            var path = 'angularShopping/src/assets/img/products/' + data[0].imageName;
                            fs.unlinkSync(path);
                        } catch (err) {
                            if (err.code === 'ENOENT') {
                    ...
                    productModel
                        .updateMany({ _id: id }, obj, (err, data) => {
                            if (err) {
                                res.send({ kq: 0, msg: 'Product name is already exists!' });
                            }
                            else {
        ...
        .exec((err, data) => {
            if (err) {
                ...
                for( var i = 0; i < data.length; i++){
                    ...
                    try {
                        var path = 'angularShopping/src/assets/img/products/' + data[i].imageName;
                        fs.unlinkSync(path);
                    } catch (err) {
                        if (err.code === 'ENOENT') {
                            console.log('File not found!');
