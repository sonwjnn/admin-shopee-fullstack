                            ...
                            jwt.sign(
                                ...
                                (err, token) => {
                                    if (err) {
                                        ...
                                        res.send({ kq: 1, msg: token, id: data[0]._id, username: data[0].username });
                                    }
                                }
                            );
                        }
        ...
        jwt.verify(token, secret, function (err, decoded) {
            if (err) {
                ...
                if (decoded.data != '') {
                    ...
                    userModel
                        .find({ $or: [{ _id: decoded.data }] })
                        .exec((err, data) => {
                            if (err) throw err;
                            else {
                                if (data.length != 0) {
                                    res.send({ kq: 1, msg: "success", username: data[0].username, id: decoded.data, data: data });
                                }
                                else {
...
router.post('/add', function (req, res) {
    ...
    if (flag == 1) {
        const obj = { name, username, password: bcrypt.hashSync(password, salt), email, phone, address, role };
